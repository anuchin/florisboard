...
for x in file_list:
    ...
    tables[schema]['VoxKBLocale'] = f"{conf['InputMethod']['LangCode']}_{schema}"
...
# Writing
language_pack = [dict(id=table['VoxKBLocale'], hanShapeBasedKeyCode=table['KeyCode']) for schema, table in tables.items()]
...
for schema, table in tables.items():
    ...
    # put_table(database, table['VoxKBLocale'], table)

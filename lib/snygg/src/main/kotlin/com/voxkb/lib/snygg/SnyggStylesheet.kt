...
data class SnyggStylesheet internal constructor(
    ...
    companion object {
        const val SCHEMA_V2 = "https://schemas.voxkb.org/snygg/v2/stylesheet"

        private val SCHEMA_PATTERN = """^https://schemas.voxkb.org/snygg/v[0-9]+/stylesheet$""".toRegex()
        ...
        fun fromJson(
            ...
            val migratedEntries = migrateNestedAttributeSelectors(jsonObject)
            val ruleMap = mutableMapOf<SnyggRule, SnyggPropertySet>()
            for ((key, jsonElement) in migratedEntries) {
                if (key == "\$schema") {
                    continue
                } else {
        ...
        private fun migrateNestedAttributeSelectors(jsonObject: JsonObject): List<Map.Entry<String, kotlinx.serialization.json.JsonElement>> {
            val result = mutableListOf<Map.Entry<String, kotlinx.serialization.json.JsonElement>>()
            for (entry in jsonObject.entries) {
                val key = entry.key
                val value = entry.value
                if (key.startsWith("\$") || key.startsWith("@") || value !is JsonObject) {
                    result.add(entry)
                    continue
                }
                val voicestate = value["voicestate"]
                if (voicestate is JsonObject) {
                    val cleaned = value.toMutableMap()
                    cleaned.remove("voicestate")
                    result.add(MapEntry(key, JsonObject(cleaned)))
                    for ((stateName, stateProps) in voicestate) {
                        if (stateProps is JsonObject) {
                            val migratedKey = "$key[voicestate=`$stateName`]"
                            result.add(MapEntry(migratedKey, stateProps))
                        }
                    }
                } else {
                    result.add(entry)

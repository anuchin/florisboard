...
class SnyggStylesheetTest {
    private val SCHEMA_LINE = """"${'$'}schema": "https://schemas.voxkb.org/snygg/v2/stylesheet""""
    ...
    inner class BasicTests {
        ...
        fun `basic deserialization`() {
            ...
            assertEquals("https://schemas.voxkb.org/snygg/v2/stylesheet", stylesheet.schema)
        ...
        fun `basic serialization`() {
            val stylesheet = SnyggStylesheet(
                schema = "https://schemas.voxkb.org/snygg/v2/stylesheet",
                rules = mapOf(
            ...
            @Language("json")
            val expectedJson =
                """{"${'$'}schema":"https://schemas.voxkb.org/snygg/v2/stylesheet","smartbar":{"background":"rgba(255,0,0,1)","shape":"rectangle()"},"key:pressed":{"shape":"circle()"}}"""
    ...
    inner class FontAnnotationTests {
        ...
        fun `serialization of valid font (with one source)`() {
            ...
            @Language("json")
            val expectedJson =
                """{"${'$'}schema":"https://schemas.voxkb.org/snygg/v2/stylesheet","@font `Comic Sans`":[{"src":"uri(`flex:/path/to/font.ttf`)"}]}"""
            val actualJson = stylesheet.toJson().getOrThrow()
            assertEquals(expectedJson, actualJson)
        }
        ...
        fun `serialization of valid font (with two sources)`() {
            ...
            @Language("json")
            val expectedJson =
                """{"${'$'}schema":"https://schemas.voxkb.org/snygg/v2/stylesheet","@font `Comic Sans`":[{"src":"uri(`flex:/path/to/font.ttf`)"},{"src":"uri(`flex:/path/to/font2.ttf`)"}]}"""
            val actualJson = stylesheet.toJson().getOrThrow()
            assertEquals(expectedJson, actualJson)
        }
    ...
    inner class LenientDeserialization {
        ...
        fun `deserialization with unsupported schema but ignoreUnsupportedSchema=true should succeed`() {
            val json = """
            {
              "${'$'}schema": "https://schemas.voxkb.org/snygg/v1/stylesheet",
              "@defines": {
                "--test": "transparent"
              },
              "element": {
    ...
    @Nested
    inner class NestedVoicestateMigration {
        @Test
        fun `nested voicestate property is migrated to attribute selector`() {
            val json = """
            {
              $SCHEMA_LINE,
              "voice-mic-button": {
            ...
            val baseRule = SnyggElementRule("voice-mic-button")
            val baseProps = stylesheet.rules[baseRule] as SnyggSinglePropertySet
            assertIs<SnyggStaticColorValue>(baseProps.background)
            assertIs<SnyggStaticColorValue>(baseProps.foreground)

            val recordingRule = SnyggElementRule(
                "voice-mic-button",
                SnyggAttributes.of("voicestate" to listOf("recording")),
            )
            val recordingProps = stylesheet.rules[recordingRule] as SnyggSinglePropertySet
            assertIs<SnyggStaticColorValue>(recordingProps.background)
            assertEquals(Color(0xFFFF5252), recordingProps.background.color)
        }

        @Test
        fun `nested voicestate with multiple states is migrated`() {
            val json = """
            {
              $SCHEMA_LINE,
              "voice-mic-button": {
            ...
            val idleRule = SnyggElementRule(
                "voice-mic-button",
                SnyggAttributes.of("voicestate" to listOf("idle")),
            )
            assertIs<SnyggSinglePropertySet>(stylesheet.rules[recordingRule])
            assertIs<SnyggSinglePropertySet>(stylesheet.rules[idleRule])
        }

        @Test
        fun `element without voicestate is not affected by migration`() {
            val json = """
            {
              $SCHEMA_LINE,
              "element": {

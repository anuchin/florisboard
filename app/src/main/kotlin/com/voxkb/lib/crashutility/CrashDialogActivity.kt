...
package com.voxkb.lib.crashutility
...
import com.voxkb.lib.devtools.Devtools
import com.voxkb.lib.devtools.LogTopic
import com.voxkb.lib.devtools.flogWarning
import kotlin.properties.ReadOnlyProperty
import kotlin.reflect.KProperty

private class SafePreferenceInstanceWrapper : ReadOnlyProperty<Any?, VoxKBPreferenceModel?> {
    ...
    override fun getValue(thisRef: Any?, property: KProperty<*>): VoxKBPreferenceModel? {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val versionName = buildString {
            ...
            if (BuildConfig.DEBUG) {
                append(stringRes(R.string.voxkb__commit_by_hash_url, "hash" to BuildConfig.BUILD_COMMIT_HASH))
            } else {
                append(stringRes(R.string.voxkb__changelog_url, "version" to BuildConfig.VERSION_NAME))
            }
            append(")")
        }
        errorReport.apply {
            appendLine("#### Environment information")
            appendLine("- VoxKB $versionName (${BuildConfig.VERSION_CODE})")
        ...
        openBugReportForm.setOnClickListener {
            val browserIntent = Intent(
                Intent.ACTION_VIEW,
                Uri.parse(resources.getString(R.string.voxkb__issue_tracker_url))
            )

/*
 * Copyright (C) 2020-2025 The VoxKB Contributors
 *
...
import com.voxkb.ime.keyboard.AbstractKeyData
import com.voxkb.ime.keyboard.CaseSelector
import com.voxkb.ime.keyboard.CharWidthSelector
import com.voxkb.ime.keyboard.KanaSelector
import com.voxkb.ime.keyboard.KeyData
import com.voxkb.ime.keyboard.LayoutDirectionSelector
import com.voxkb.ime.keyboard.ShiftStateSelector
import com.voxkb.ime.keyboard.VariationSelector
import com.voxkb.ime.text.keyboard.AutoTextKeyData
import com.voxkb.ime.text.keyboard.MultiTextKeyData
...
import com.voxkb.lib.android.reader
import com.voxkb.lib.kotlin.resultErr
import com.voxkb.lib.kotlin.resultErrStr
import com.voxkb.lib.kotlin.resultOk
...
fun VoxKBRef.delete(context: Context) {
    when {
        isCache || isInternal -> {
...
fun VoxKBRef.hasAsset(context: Context): Boolean {
    return when {
        isAssets -> {
            try {
...
fun VoxKBRef.list(context: Context) = list(context, files = true, dirs = true)

fun VoxKBRef.listFiles(context: Context) = list(context, files = true, dirs = false)

fun VoxKBRef.listDirs(context: Context) = list(context, files = false, dirs = true)

private fun VoxKBRef.list(appContext: Context, files: Boolean, dirs: Boolean) = runCatching<List<VoxKBRef>> {
    when {
        !files && !dirs -> listOf()
        isAssets -> {
        ...
        else -> error("Unsupported VoxKBRef source!")
    }
}

fun <T> VoxKBRef.loadJsonAsset(
    context: Context,
...
fun VoxKBRef.loadTextAsset(context: Context): Result<String> {
    return when {

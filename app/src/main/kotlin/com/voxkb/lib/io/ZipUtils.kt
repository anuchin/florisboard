...
import com.voxkb.lib.android.copyRecursively
import com.voxkb.lib.android.write
...
object ZipUtils {
    fun readFileFromArchive(context: Context, zipRef: VoxKBRef, relPath: String) = runCatching<String> {
        when {
            zipRef.isAssets -> {
    ...
    fun zip(context: Context, srcRef: VoxKBRef, dstRef: VoxKBRef) =
        zip(context, FsDir(srcRef.absolutePath(context)), dstRef)

    fun zip(context: Context, srcDir: FsDir, dstRef: VoxKBRef) = runCatching {
    ...
    fun unzip(context: Context, srcRef: VoxKBRef, dstRef: VoxKBRef) =
        unzip(context, srcRef, FsDir(dstRef.absolutePath(context)))

    fun unzip(context: Context, srcRef: VoxKBRef, dstDir: FsFile) = runCatching {

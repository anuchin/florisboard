...
import com.voxkb.lib.android.AndroidSettings
import com.voxkb.lib.android.AndroidVersion
import com.voxkb.lib.android.systemServiceOrNull
import com.voxkb.lib.compose.observeAsState

private const val DELIMITER = ':'
private const val IME_SERVICE_CLASS_NAME = "com.voxkb.VoxKBImeService"
private const val TIMED_QUERY_DELAY = 500L

object InputMethodUtils {
    fun isVoxKBboardEnabled(context: Context): Boolean {
        return if (AndroidVersion.ATLEAST_API34_U) {
            ...
            enabledImeList != null && parseIsVoxKBboardEnabled(context, enabledImeList)
        }
    }

    fun isVoxKBboardSelected(context: Context): Boolean {
        return if (AndroidVersion.ATLEAST_API34_U) {
            context.systemServiceOrNull(InputMethodManager::class)
            ...
            selectedIme != null && parseIsVoxKBboardSelected(context, selectedIme)
        }
    }

    @Composable
    fun observeIsVoxKBboardEnabled(
        context: Context = LocalContext.current.applicationContext,
        foregroundOnly: Boolean = false,
    ): State<Boolean> {
        return if (AndroidVersion.ATLEAST_API34_U) {
            timedObserveIsVoxKBEnabled()
        } else {
            AndroidSettings.Secure.observeAsState(
                key = Settings.Secure.ENABLED_INPUT_METHODS,
                foregroundOnly = foregroundOnly,
                transform = { parseIsVoxKBboardEnabled(context, it.toString()) },
            )
        }
    }

    @Composable
    fun observeIsVoxKBboardSelected(
        context: Context = LocalContext.current.applicationContext,
        foregroundOnly: Boolean = false,
    ): State<Boolean> {
        return if (AndroidVersion.ATLEAST_API34_U) {
            timedObserveIsVoxKBSelected()
        } else {
            AndroidSettings.Secure.observeAsState(
                key = Settings.Secure.DEFAULT_INPUT_METHOD,
                foregroundOnly = foregroundOnly,
                transform = { parseIsVoxKBboardSelected(context, it.toString()) },
            )
        }
    }

    fun parseIsVoxKBboardEnabled(context: Context, activeImeIds: String): Boolean {
    ...
    fun parseIsVoxKBboardSelected(context: Context, selectedImeId: String): Boolean {
    ...
    @RequiresApi(api = 34)
    @Composable
    private fun timedObserveIsVoxKBEnabled(): State<Boolean> {
        val state = remember { mutableStateOf(false) }
        val context = LocalContext.current
        LaunchedEffect(Unit) {
            while (true) {
                state.value = isVoxKBboardEnabled(context)
    ...
    @RequiresApi(api = 34)
    @Composable
    private fun timedObserveIsVoxKBSelected(): State<Boolean> {
        val state = remember { mutableStateOf(false) }
        val context = LocalContext.current
        LaunchedEffect(Unit) {
            while (true) {
                state.value = isVoxKBboardSelected(context)
                delay(TIMED_QUERY_DELAY)
            }
        }
        return state

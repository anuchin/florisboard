...
import com.voxkb.lib.ValidationRule
import com.voxkb.lib.android.readText
import com.voxkb.lib.android.writeText
...
interface UserDictionaryDao {
    ...
    fun query(word: String, locale: VoxKBLocale?): List<UserDictionaryEntry>
    ...
    fun queryShortcut(shortcut: String, locale: VoxKBLocale?): List<UserDictionaryEntry>
    ...
    fun queryAll(locale: VoxKBLocale?): List<UserDictionaryEntry>
    ...
    fun queryExact(word: String, locale: VoxKBLocale?): List<UserDictionaryEntry>

    @Query("$SELECT_ALL_FROM_WORDS WHERE ${UserDictionary.Words.WORD} = :word AND $LOCALE_MATCHES")
    fun queryExactFuzzyLocale(word: String, locale: VoxKBLocale?): List<UserDictionaryEntry>

    @Query("SELECT DISTINCT ${UserDictionary.Words.LOCALE} FROM $WORDS_TABLE")
    fun queryLanguageList(): List<VoxKBLocale?>
            ...
            src.forEachLine { line ->
                if (isFirstLine) {
                    ...
                    val alreadyExistingEntries = userDictionaryDao().queryExact(
                        word, locale?.let { VoxKBLocale.fromTag(it) },
                    )
...
@TypeConverters(VoxKBUserDictionaryDatabase.Converters::class)
abstract class VoxKBUserDictionaryDatabase : RoomDatabase(), UserDictionaryDatabase {
    companion object {
        const val DB_FILE_NAME = "voxkb_user_dictionary"
    }
    ...
    class Converters {
        @TypeConverter
        fun localeToString(locale: VoxKBLocale?): String? {
            return when (locale) {
                null -> null
        ...
        @TypeConverter
        fun stringToLocale(string: String?): VoxKBLocale? {
            return when (string) {
                null, "all", "null", "" -> null
                else -> VoxKBLocale.fromTag(string)
            }
        }
    }
}
...
class SystemUserDictionaryDatabase(context: Context) : UserDictionaryDatabase {
    ...
    private val dao = object : UserDictionaryDao {
        ...
        override fun query(word: String, locale: VoxKBLocale?): List<UserDictionaryEntry> {
        ...
        override fun queryShortcut(shortcut: String, locale: VoxKBLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryAll(locale: VoxKBLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryExact(word: String, locale: VoxKBLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryExactFuzzyLocale(word: String, locale: VoxKBLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
                    selection = "${UserDictionary.Words.WORD} = ? AND ${UserDictionary.Words.LOCALE} IS NULL",
                    selectionArgs = arrayOf(word),
        ...
        override fun queryLanguageList(): List<VoxKBLocale?> {
            ...
            val retList = mutableSetOf<VoxKBLocale?>()
            while (cursor.moveToNext()) {
                val localeStr = cursor.getString(localeIndex)
                if (localeStr == null) {
                    retList.add(null)
                } else {
                    retList.add(VoxKBLocale.fromTag(localeStr))
                }
            }
            cursor.close()
...
object UserDictionaryValidation {
    ...
    val Locale = ValidationRule<String> {
        ...
        validator { input ->
            ...
            when {
                input.isBlank() -> resultValid() // Is optional
                tryOrNull { VoxKBLocale.fromTag(str) } == null -> resultInvalid(error = R.string.settings__udm__dialog__locale_error_invalid)
                else -> resultValid()
            }
        }
    }

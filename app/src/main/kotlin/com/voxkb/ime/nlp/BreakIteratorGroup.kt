...
open class BreakIteratorGroup {
    private val charInstances = Cache.Builder<VoxKBLocale, GuardedByLock<BreakIterator>>().build()

    private val wordInstances = Cache.Builder<VoxKBLocale, GuardedByLock<BreakIterator>>().build()

    private val sentenceInstances = Cache.Builder<VoxKBLocale, GuardedByLock<BreakIterator>>().build()

    suspend fun <R> character(locale: VoxKBLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun <R> word(locale: VoxKBLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun <R> sentence(locale: VoxKBLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun measureUChars(
        text: String,
        numUnicodeChars: Int,
        locale: VoxKBLocale = VoxKBLocale.default(),
    ): Int {

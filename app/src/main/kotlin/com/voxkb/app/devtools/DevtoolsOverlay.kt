...
package com.voxkb.app.devtools
...
import com.voxkb.ime.keyboard.CachedLayout
import com.voxkb.ime.keyboard.DebugLayoutComputationResult
...
import com.voxkb.lib.snygg.SnyggMissingSchemaException

private val CardBackground = Color.Black.copy(0.6f)
private val DateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH-mm-ss", VoxKBLocale.default().base)
    ...
    DevtoolsOverlayBox(title = "Failed to load stylesheet, fell back to base style") {
        ...
        if (cause is SnyggMissingSchemaException) {
            DevtoolsSubGroup(title = "Explanation") {
                DevtoolsText(
                    text = """
                    It appears you’re trying to load a theme designed for VoxKB v0.4 (Snygg v1), which isn’t compatible with the latest release using Snygg v2.

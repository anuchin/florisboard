...
import com.voxkb.R
import com.voxkb.app.enumDisplayEntriesOf
import com.voxkb.ime.input.InputKeyEventReceiver
import com.voxkb.ime.input.InputShiftState
import com.voxkb.ime.keyboard.ComputingEvaluator
import com.voxkb.ime.keyboard.DefaultComputingEvaluator
import com.voxkb.ime.keyboard.Key
import com.voxkb.ime.keyboard.KeyData
import com.voxkb.ime.keyboard.Keyboard
import com.voxkb.ime.keyboard.KeyboardMode
import com.voxkb.ime.keyboard.computeImageVector
import com.voxkb.ime.keyboard.computeLabel
import com.voxkb.ime.text.key.KeyCode
import com.voxkb.ime.text.keyboard.TextKeyData
import com.voxkb.ime.theme.VoxKBImeUi
import com.voxkb.ime.window.ImeWindowMode
import com.voxkb.keyboardManager
import com.voxkb.lib.NATIVE_NULLPTR
import com.voxkb.lib.compose.VoxKBHyperlinkText
...
import com.voxkb.lib.compose.VoxKBChip
...
import com.voxkb.lib.kotlin.curlyFormat
import com.voxkb.lib.snygg.SnyggAnnotationRule
import com.voxkb.lib.snygg.SnyggAttributes
import com.voxkb.lib.snygg.SnyggElementRule
import com.voxkb.lib.snygg.SnyggRule
import com.voxkb.lib.snygg.SnyggSelector
import com.voxkb.lib.snygg.ui.NonNullSaver
...
internal fun EditRuleDialog(
    ...
    val possibleRuleTemplates = remember {
        buildList {
            ...
            VoxKBImeUi.elementNames.forEach { name ->
        ...
        Column {
            ...
            (currentRule as? SnyggElementRule)?.apply {
                ...
                DialogProperty(text = stringRes(R.string.settings__theme_editor__rule_selectors)) {
                    Row(
                        modifier = Modifier.voxkbHorizontalScroll(),
                ...
                val codes = remember(currentRule) {
                    attributes[VoxKBImeUi.Attr.Code] ?: emptyList()
                }
                ...
                if (initCodeValue != null) {
                    EditCodeValueDialog(
                        ...
                        onAdd = {
                            currentRule = copy(
                                attributes = attributes.including(VoxKBImeUi.Attr.Code to it)
                            )
                        },
                        onDelete = {
                            currentRule = copy(
                                attributes = attributes.excluding(VoxKBImeUi.Attr.Code to it)
                            )
                        },
                ...
                EnumLikeAttributeBox(
                    ...
                    attribute = VoxKBImeUi.Attr.Mode,
                ...
                EnumLikeAttributeBox(
                    ...
                    attribute = VoxKBImeUi.Attr.ShiftState,
                    attributes = attributes,
                    setAttributes = { currentRule = copy(attributes = it) },
                    level = level,
                )
                ...
                EnumLikeAttributeBox(
                    ...
                    attribute = VoxKBImeUi.Attr.WindowMode,
                    attributes = attributes,
                    setAttributes = { currentRule = copy(attributes = it) },
                    level = level,
                )
...
private fun EditCodeValueDialog(
    ...
    val isVoxKBEnabled by InputMethodUtils.observeIsVoxKBboardEnabled(foregroundOnly = true)
    val isVoxKBSelected by InputMethodUtils.observeIsVoxKBboardSelected(foregroundOnly = true)
    ...
    fun requestStartRecording() {
        ...
        if (!isVoxKBEnabled || !isVoxKBSelected) {
            lastRecordingToast?.cancel()
            lastRecordingToast = context.showShortToastSync(
                R.string.settings__theme_editor__code_recording_requires_default_ime_voxkb,
                "app_name" to context.stringRes(R.string.voxkb_app_name),
            )
            ...
            AnimatedVisibility(visible = showKeyCodesHelp) {
                Column(modifier = Modifier.padding(bottom = 16.dp)) {
                    ...
                    VoxKBHyperlinkText(
                        ...
                        url = stringRes(R.string.voxkb__character_key_codes_url),
                    )
                    VoxKBHyperlinkText(
                        text = "Internal (github.com)",
                        url = stringRes(R.string.voxkb__internal_key_codes_url),
                    )
                }
            }

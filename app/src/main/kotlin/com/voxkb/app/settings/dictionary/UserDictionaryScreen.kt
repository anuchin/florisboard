...
package com.voxkb.app.settings.dictionary
...
import com.voxkb.ime.dictionary.FREQUENCY_MAX
import com.voxkb.ime.dictionary.FREQUENCY_MIN
import com.voxkb.ime.dictionary.UserDictionaryDao
import com.voxkb.ime.dictionary.UserDictionaryEntry
import com.voxkb.ime.dictionary.UserDictionaryValidation
...
private val AllLanguagesLocale = VoxKBLocale.from(language = "zz")
...
@Composable
fun UserDictionaryScreen(type: UserDictionaryType) = VoxKBScreen {
    title = stringRes(when (type) {
        UserDictionaryType.FLORIS -> R.string.settings__udm__title_voxkb
    ...
    var currentLocale by remember { mutableStateOf<VoxKBLocale?>(null) }
    var languageList by remember { mutableStateOf(emptyList<VoxKBLocale>()) }
    ...
    fun userDictionaryDao(): UserDictionaryDao? {
        return when (type) {
            UserDictionaryType.FLORIS -> dictionaryManager.voxkbUserDictionaryDao()
            UserDictionaryType.SYSTEM -> dictionaryManager.systemUserDictionaryDao()
        }
    }

    fun getDisplayNameForLocale(locale: VoxKBLocale): String {
    ...
    val importDictionary = rememberLauncherForActivityResult(
        ...
        onResult = { uri ->
            ...
            val db = when (type) {
                UserDictionaryType.FLORIS -> dictionaryManager.voxkbUserDictionaryDatabase()
                    ...
                    if (isInvalid) {
                        ...
                        val entry = UserDictionaryEntry(
                            ...
                            locale = locale.trim().takeIf { it.isNotBlank() }?.let {
                                // Normalize tag
                                VoxKBLocale.fromTag(it).localeTag()
                            },
                        )
                        if (isAddWord) {

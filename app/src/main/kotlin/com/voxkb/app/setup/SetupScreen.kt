...
import com.voxkb.lib.compose.VoxKBScreenScope
...
import com.voxkb.lib.compose.VoxKBBulletSpacer
import com.voxkb.lib.compose.VoxKBStep
import com.voxkb.lib.compose.VoxKBStepLayout
import com.voxkb.lib.compose.VoxKBStepState
import com.voxkb.lib.compose.stringRes

@Composable
fun SetupScreen() = VoxKBScreen {
...
@Composable
private fun VoxKBScreenScope.content(
    isVoxKBEnabled: Boolean,
    isVoxKBSelected: Boolean,
    context: Context,
    ...
    val stepState = rememberSaveable(saver = VoxKBStepState.Saver) {
        val initStep = when {
            !isVoxKBEnabled -> Steps.EnableIme.id
            !isVoxKBSelected -> Steps.SelectIme.id
        ...
        VoxKBStepState.new(init = initStep)
    }

    content {
        LaunchedEffect(isVoxKBEnabled, isVoxKBSelected, hasNotificationPermission) {
        ...
        LaunchedEffect(Unit) {
            while (true) {
                delay(200L)
                val isEnabled = InputMethodUtils.isVoxKBboardEnabled(context)
                if (stepState.getCurrentAuto().value == Steps.EnableIme.id &&
                    ...
                    context.launchActivity(VoxKBAppActivity::class) {
...
private fun footer(context: Context) {
    ...
    Row(
        ...
        val privacyPolicyUrl = stringRes(R.string.voxkb__privacy_policy_url)
        TextButton(onClick = { context.launchUrl(privacyPolicyUrl) }) {
            Text(text = stringRes(R.string.setup__footer__privacy_policy))
        }
        VoxKBBulletSpacer()
        val repositoryUrl = stringRes(R.string.voxkb__repo_url)
...
@Composable
private fun PreferenceUiScope<VoxKBPreferenceModel>.steps(
    context: Context,
    navController: NavController,
    requestNotification: ManagedActivityResultLauncher<String, Boolean>,
    scope: CoroutineScope,
    stepState: VoxKBStepState,
): List<VoxKBStep> {
